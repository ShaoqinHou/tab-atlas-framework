...
import {
  ...
  createYouTubeLayeredEvidenceAdapter,
...
describe('standardized YouTube evidence', () => {
  ...
  it('parses yt-dlp VTT subtitle artifacts into transcript segments', () => {
    const transcript = selectYtDlpTranscript({
      subtitles: {
        en: [{
          ext: 'vtt',
          data: [
            ...
            'Manual <b>subtitle</b> &amp; cue',
          ].join('\n'),
        }],
      },
    }, { localYtDlp: { enabled: true, executable: 'yt-dlp', allowAutomaticCaptions: false, preferredLanguages: ['en'] } });

    expect(transcript.status).toBe('available_artifact');
    expect(transcript.segments[0]).toEqual({
      startSeconds: 1,
      durationSeconds: 2.5,
      text: 'Manual subtitle & cue',
    });
    expect(transcript.plainText).toBe('Manual subtitle & cue');
  });

  it('parses yt-dlp JSON3 subtitle artifacts into transcript segments', () => {
    const transcript = selectYtDlpTranscript({
      automatic_captions: {
        en: [{
          ext: 'json3',
  ...
  it('layers official metadata with local yt-dlp transcript evidence', async () => {
    const previous = process.env.YOUTUBE_API_KEY;
    process.env.YOUTUBE_API_KEY = 'test-key';
    const adapter = createYouTubeLayeredEvidenceAdapter({
      officialDataApi: { enabled: true, apiKeyEnvironmentVariable: 'YOUTUBE_API_KEY' },
      localYtDlp: { enabled: true, executable: 'yt-dlp', allowAutomaticCaptions: false, preferredLanguages: ['en'] },
    }, {
      apiBaseUrl: 'https://youtube.test/videos',
    ...
    const payload = result.artifacts[0].jsonPayload as {
      title: string;
      channelTitle: string;
      transcript: { status: string; plainText: string };
    };
    expect(result.status).toBe('complete');
    expect(payload.title).toBe('Official title');
    expect(payload.channelTitle).toBe('Official channel');
    expect(payload.transcript.status).toBe('available_artifact');
    expect(payload.transcript.plainText).toBe('Local transcript line');
    expect(result.warnings).toEqual(expect.arrayContaining([
      'official metadata layer complete',
